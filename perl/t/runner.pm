...
sub env_override {
  my ($m) = @_;
  my $live = getenv('UNIVEC_TEST_LIVE');
  my $override = getenv('UNIVEC_TEST_OVERRIDE');
  ...
  my $explain = getenv('UNIVEC_TEST_EXPLAIN');
  $m->{'UNIVEC_TEST_EXPLAIN'} = $explain if defined $explain && '' ne $explain;
...
sub is_control_skipped {
  ...
  my $skip = UnivecHelpers::gpath($ctrl, "test.skip.$mode") || {};
  my $items = $skip->{$kind} || [];
...
sub live_client_options {
  my $ctrl = load_test_control();
  my $opts = UnivecHelpers::gpath($ctrl, 'test.client.options');
...
sub live_delay_ms {
  my $ctrl = load_test_control();
  my $v = UnivecHelpers::gpath($ctrl, 'test.live.delayMs');
