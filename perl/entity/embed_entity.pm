...
sub make {
  ...
  return EmbedEntity->new($self->{_client}, $opts);
}
...
# Returns the current Embed data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Embed fields).
sub match_get {
  my ($self) = @_;
...
# Create a new Embed.
#
# reqdata: body data hashref (EmbedCreateData shape). ctrl: optional
# per-call control. Returns the created Embed data (hashref); dies
