...
sub make {
  ...
  return ConvertEntity->new($self->{_client}, $opts);
}
...
# Returns the current Convert data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Convert fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (ConvertCreateData shape). ctrl: optional
# per-call control. Returns the created Convert data (hashref); dies
