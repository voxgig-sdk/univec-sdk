...
sub new {
  ...
  elsif (UnivecHelpers::is_false($entopts->{active})) {
    # keep false
  }
  else {
    $entopts->{active} = Voxgig::Struct::JTRUE();
...
sub make {
  ...
  return EphemeralKeyEntity->new($self->{_client}, $opts);
}
...
sub data_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_data} = UnivecHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetData');
  }
  return;
}

# Returns the current EphemeralKey data (hashref).
sub data_get {
  my ($self) = @_;
...
sub match_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_match} = UnivecHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetMatch');
  }
  return;
}

# Returns the current match filter (any subset of EphemeralKey fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (EphemeralKeyCreateData shape). ctrl: optional
# per-call control. Returns the created EphemeralKey data (hashref); dies
# with UnivecError on failure.
sub create {
  ...
  return $self->_run_op($ctx, sub {
    ...
    if ($result) {
      if ($result->{resdata}) {
        $self->{_data} = UnivecHelpers::to_map(
...
sub stream {
  ...
  my $utility = $self->{_utility};
  $callopts = UnivecHelpers::to_map($callopts) || {};
  my $signal = UnivecHelpers::gp($callopts, 'signal');
  my $ctrl = UnivecHelpers::to_map(
    UnivecHelpers::gp($callopts, 'ctrl')) || {};
