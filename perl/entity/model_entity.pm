...
sub make {
  ...
  return ModelEntity->new($self->{_client}, $opts);
}
...
# Returns the current Model data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Model fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match filter hashref (any subset of Model fields;
# ModelListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Model items as an arrayref; dies with
# UnivecError on failure.
sub list {
