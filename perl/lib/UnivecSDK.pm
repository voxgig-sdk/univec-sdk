...
sub new {
  ...
  my $config = UnivecConfig::shared_config();
  ...
  if (UnivecHelpers::is_true(
    UnivecHelpers::gpath($self->{options}, 'feature.test.active'))) {
    $self->{mode} = 'test';
  }
  ...
  my $feature_opts = UnivecHelpers::to_map(
    UnivecHelpers::gp($self->{options}, 'feature')) || {};
  my $featureorder = UnivecHelpers::gpath(
    $self->{options}, '__derived__.featureorder');
  $featureorder = [] unless Voxgig::Struct::islist($featureorder);
  for my $fname (@$featureorder) {
    my $fopts = UnivecHelpers::to_map($feature_opts->{$fname});
    if ($fopts && UnivecHelpers::is_true($fopts->{active})) {
      $utility->{feature_add}->($self->{_rootctx},
        UnivecFeatures::make_feature($fname));
    }
  }

  # Add extension features.
  my $extend = UnivecHelpers::gp($self->{options}, 'extend');
...
sub get_utility {
  my ($self) = @_;
  return UnivecUtility->copy($self->{_utility});
}
...
sub prepare {
  ...
  my $ctrl = UnivecHelpers::to_map(
    UnivecHelpers::gp($fetchargs, 'ctrl')) || {};
  ...
  my $path = UnivecHelpers::gp($fetchargs, 'path');
  $path = '' unless defined $path && !ref $path;
  my $method_val = UnivecHelpers::gp($fetchargs, 'method');
  $method_val = 'GET' unless defined $method_val && !ref $method_val;
  my $params = UnivecHelpers::to_map(
    UnivecHelpers::gp($fetchargs, 'params')) || {};
  my $query = UnivecHelpers::to_map(
    UnivecHelpers::gp($fetchargs, 'query')) || {};
  my $headers = $utility->{prepare_headers}->($ctx);

  my $base = UnivecHelpers::gp($opts, 'base');
  $base = '' unless defined $base && !ref $base;
  my $prefix = UnivecHelpers::gp($opts, 'prefix');
  $prefix = '' unless defined $prefix && !ref $prefix;
  my $suffix = UnivecHelpers::gp($opts, 'suffix');
  ...
  $ctx->{spec} = UnivecSpec->new({
    ...
    'body' => UnivecHelpers::gp($fetchargs, 'body'),
    'step' => 'start',
  });

  # Merge user-provided headers.
  my $uh = UnivecHelpers::gp($fetchargs, 'headers');
...
sub _op_allowed {
  my ($self, $op) = @_;
  my $allow = UnivecHelpers::gpath($self->{options}, 'allow.op');
...
sub _op_denied {
  ...
  return {
    'ok' => 0,
    'err' => "UnivecSDK: $op: operation not allowed by" .
...
sub _raw_request {
  ...
  if (Voxgig::Struct::ismap($fetched)) {
    my $status = UnivecHelpers::to_int(
      UnivecHelpers::gp($fetched, 'status'));
    my $headers = UnivecHelpers::gp($fetched, 'headers') || {};
    ...
    my $json_data;
    unless ($no_body) {
      my $jf = UnivecHelpers::gp($fetched, 'json');
      if (ref $jf eq 'CODE') {
...
sub graphql {
  ...
  my $errors = UnivecHelpers::gpath($res, 'data.errors');
...
sub Convert {
  my ($self, $data) = @_;
  require(Cwd::abs_path("$DIR/../entity/convert_entity.pm"));
  return ConvertEntity->new($self, $data);
}


# Canonical facade: $client->Embed->list / ->load({ 'id' => ... })
sub Embed {
  my ($self, $data) = @_;
  require(Cwd::abs_path("$DIR/../entity/embed_entity.pm"));
  return EmbedEntity->new($self, $data);
}
...
sub EphemeralKey {
  my ($self, $data) = @_;
  require(Cwd::abs_path("$DIR/../entity/ephemeral_key_entity.pm"));
  return EphemeralKeyEntity->new($self, $data);
}
...
sub Model {
  my ($self, $data) = @_;
  require(Cwd::abs_path("$DIR/../entity/model_entity.pm"));
  return ModelEntity->new($self, $data);
}
