...
class UnivecTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            UnivecTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = UnivecTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = UnivecTestRunner.getenv("UNIVEC_TEST_LIVE")
        override = UnivecTestRunner.getenv("UNIVEC_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = UnivecTestRunner.getenv(key)
        ...
        explain = UnivecTestRunner.getenv("UNIVEC_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["UNIVEC_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if UnivecTestRunner._test_control is not None:
            return UnivecTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                UnivecTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            UnivecTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = UnivecTestRunner.load_test_control()
...
def env_override(m):
    return UnivecTestRunner.env_override(m)


def entity_data(v):
    return UnivecTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return UnivecTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return UnivecTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return UnivecTestRunner.load_test_control()


def live_client_options():
    return UnivecTestRunner.live_client_options()


def live_delay_ms():
    return UnivecTestRunner.live_delay_ms()
