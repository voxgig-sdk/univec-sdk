...
class TestEmbedEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Embed(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _embed_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "embed." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set UNIVEC_TEST_EMBED_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        embed_ref01_ent = client.Embed(None)
...
def _embed_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/embed/EmbedTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("UNIVEC_TEST_EMBED_ENTID"))
