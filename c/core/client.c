// UnivecSDK client (generated — mirrors the rust Main fragment).
...
UnivecSDK* univec_sdk_new(voxgig_value* options) {
  UnivecSDK* sdk = (UnivecSDK*)calloc(1, sizeof(UnivecSDK));
...
static bool sdk_op_allowed(UnivecSDK* sdk, const char* op) {
...
static voxgig_value* sdk_op_denied(UnivecSDK* sdk, const char* op) {
  voxgig_value* allow_op = getpath2(sdk->options, "allow", "op");
...
static voxgig_value* sdk_raw_request(
  UnivecSDK* sdk, voxgig_value* fetchargs, PNError** err) {
  *err = NULL;
  Utility* utility = sdk->utility;
...
voxgig_value* sdk_graphql(UnivecSDK* sdk, const char* query,
  ...
  *err = NULL;
  ...
  if (voxgig_is_list(errors) && 0 < voxgig_as_list(errors)->len) {
    ...
    char msg[512];
    snprintf(msg, sizeof(msg), "UnivecSDK: graphql: %s", m);
...
UnivecSDK* test_sdk(voxgig_value* testopts, voxgig_value* sdkopts) {
  ...
  UnivecSDK* sdk = univec_sdk_new(sdkopts);
  free(sdk->mode);
  sdk->mode = strdup("test");
  return sdk;
}
