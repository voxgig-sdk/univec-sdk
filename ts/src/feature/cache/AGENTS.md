# CacheFeature — Agent Guide

Response caching for safe read requests (v0.0.1).
...
runtime for the `cache` feature in the TypeScript target — do
...
| Part | Path |
| --- | --- |
| Model definition | `.sdk/model/feature/cache.aon` (name, title, version, `config.options.active`, the `hook` map, per-language `deps`) |
| Registered in | `.sdk/model/feature/feature-index.aon` (`@"cache.aon"`) |
| Runtime template | `.sdk/tm/ts/src/feature/cache/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Turn hooks on/off**: edit the `hook` map in
  `.sdk/model/feature/cache.aon` (`<Stage>: active: true|false`).
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/ts/src/feature/cache/`, then regenerate.
