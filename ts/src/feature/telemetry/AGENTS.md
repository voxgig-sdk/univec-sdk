...
runtime for the `telemetry` feature in the TypeScript target — do
...
| Runtime template | `.sdk/tm/ts/src/feature/telemetry/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/ts/src/feature/telemetry/`, then regenerate.
