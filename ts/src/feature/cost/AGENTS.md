# CostFeature — Agent Guide

Cost tracking and spend budget for API calls (v0.0.1).
...
runtime for the `cost` feature in the TypeScript target — do
...
| Part | Path |
| --- | --- |
| Model definition | `.sdk/model/feature/cost.aon` (name, title, version, `config.options.active`, the `hook` map, per-language `deps`) |
| Registered in | `.sdk/model/feature/feature-index.aon` (`@"cost.aon"`) |
| Runtime template | `.sdk/tm/ts/src/feature/cost/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Turn hooks on/off**: edit the `hook` map in
  `.sdk/model/feature/cost.aon` (`<Stage>: active: true|false`).
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/ts/src/feature/cost/`, then regenerate.
