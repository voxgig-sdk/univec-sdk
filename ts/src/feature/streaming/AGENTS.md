# StreamingFeature — Agent Guide

Incremental streaming of list results via async iteration (v0.0.1).
...
runtime for the `streaming` feature in the TypeScript target — do
...
| Part | Path |
| --- | --- |
| Model definition | `.sdk/model/feature/streaming.aon` (name, title, version, `config.options.active`, the `hook` map, per-language `deps`) |
| Registered in | `.sdk/model/feature/feature-index.aon` (`@"streaming.aon"`) |
| Runtime template | `.sdk/tm/ts/src/feature/streaming/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Turn hooks on/off**: edit the `hook` map in
  `.sdk/model/feature/streaming.aon` (`<Stage>: active: true|false`).
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/ts/src/feature/streaming/`, then regenerate.
