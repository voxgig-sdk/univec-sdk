...
runtime for the `proxy` feature in the TypeScript target — do
...
| Runtime template | `.sdk/tm/ts/src/feature/proxy/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/ts/src/feature/proxy/`, then regenerate.
