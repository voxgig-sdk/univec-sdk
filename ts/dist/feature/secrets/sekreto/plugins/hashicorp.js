...
exports.hashicorpprovider = hashicorpprovider;
...
function hashicorpprovider(addr, token, options) {
    ...
    if (1 !== kv && 2 !== kv) {
        throw new support_1.SekretoError('sekreto: hashicorp: unsupported kv version: ' + String(kv));
    }
    ...
    const login = async () => {
        ...
        if (!auth) {
            throw new support_1.SekretoError('sekreto: hashicorp: no token and no auth method');
        }
        ...
        if ('kubernetes' === auth.method) {
            ...
            if (undefined === jwt) {
                ...
                try {
                    ...
                    throw new support_1.SekretoError('sekreto: hashicorp: cannot read jwt file ' + file);
                }
            }
            ...
            throw new support_1.SekretoError('sekreto: hashicorp: unknown auth method: ' + String(auth.method));
        }
        const res = await (0, httpjson_1.fetchjson)('POST', url, baseheaders(), JSON.stringify(body));
        const got = res.body && res.body.auth && res.body.auth.client_token;
        if (200 !== res.status || !got) {
            throw new support_1.SekretoError('sekreto: hashicorp login failed: ' + res.status + ': ' + url);
        }
    ...
    return {
        lookup: async (name) => {
            ...
            const ref = (0, support_1.vaultref)(name);
            const base = addr.replace(/\/$/, '') + '/v1/' + usemount;
            const url = 1 === kv ? base + '/' + ref.path : base + '/data/' + ref.path;
            const headers = baseheaders();
            headers['X-Vault-Token'] = livetoken;
            const res = await (0, httpjson_1.fetchjson)('GET', url, headers);
            if (404 === res.status) {
                return undefined;
            }
            if (200 !== res.status) {
                throw new support_1.SekretoError('sekreto: hashicorp error: ' + res.status + ': ' + url);
            }
            const data = 1 === kv ? res.body && res.body.data : res.body && res.body.data && res.body.data.data;
...
exports.hashicorp = (0, support_1.providerplugin)('hashicorp', (spec) => hashicorpprovider(spec.addr || '', spec.token || '', {
    mount: spec.mount,
    kv: spec.kv,
