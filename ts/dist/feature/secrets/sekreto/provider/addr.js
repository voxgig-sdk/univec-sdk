...
function checkaddr(addr) {
    const scheme = addr.startsWith('https://')
        ...
        : addr.startsWith('http://')
            ...
            : '';
    if ('' === scheme) {
        throw new support_1.SekretoError('sekreto: not an http(s) address: ' + safeaddr(addr));
    }
    ...
    if (authority.includes('@')) {
        throw new support_1.SekretoError('sekreto: refusing an address with embedded credentials: ' + safeaddr(addr));
    }
    ...
    if (authority.startsWith('[') && !authority.includes(']')) {
        throw new support_1.SekretoError('sekreto: not a valid http(s) address: ' + safeaddr(addr));
    }
    if ('https://' === scheme) {
        return;
    }
    ...
    throw new support_1.SekretoError('sekreto: refusing to send a token in plaintext to ' + safeaddr(addr) + ' (use https)');
}
