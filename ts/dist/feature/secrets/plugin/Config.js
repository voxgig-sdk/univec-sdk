...
function normalizeconfig(input) {
    ...
    for (const r of Object.keys(base.map))
        checkreserved(r, reserved);
    for (const r of Object.keys(over.map))
        checkreserved(r, reserved);
    for (const n of Object.keys(basedef))
        checkreserved(n, reserved);
    for (const n of Object.keys(overdef))
        checkreserved(n, reserved);
    ...
    const order = [];
    for (const r of over.order)
        if (-1 === order.indexOf(r))
            order.push(r);
    ...
    for (let i = 0; i < order.length; i++) {
        ...
        const layers = [];
        const nm = (0, Ref_1.parseref)(ref).name;
        if (basedef[nm] && undefined !== basedef[nm].options)
            layers.push(basedef[nm].options);
        if (b && undefined !== b.options)
            layers.push(b.options);
        if (overdef[nm] && undefined !== overdef[nm].options)
            layers.push(overdef[nm].options);
        if (o && undefined !== o.options)
            layers.push(o.options);
        const ent = { pos: i, active, start, optionlayers: layers };
        if (undefined !== ord)
            ent.order = ord;
        instance[ref] = ent;
    }
...
function entries(src) {
    ...
    if (Array.isArray(src)) {
        for (const item of src) {
            const ref = (0, Ref_1.canonref)(item.ref);
            map[ref] = item;
            order.push(ref);
        }
    }
        ...
        for (const key of Object.keys(src)) {
            map[(0, Ref_1.canonref)(key)] = src[key];
        }
...
function checkreserved(ref, reserved) {
    if (0 === reserved.length)
        return;
    const name = (0, Ref_1.parseref)(ref).name;
...
function resolveoptions(input) {
    ...
    checkshape(shape);
    const ref = (0, Ref_1.canonref)(input.ref);
    const name = (0, Ref_1.parseref)(ref).name;
    const doc = input.doc || {};
...
function defaultsof(shape) {
    const out = {};
    for (const k of Object.keys(shape)) {
        const v = shape[k];
        if (v && 'object' === typeof v && !Array.isArray(v) && undefined !== v.$MERGE)
            continue;
        out[k] = v;
    }
    return out;
}
function optsof(src, key) {
    if (null == src)
        return undefined;
    // The array form is equivalent to the map form (§9.1).
    if (Array.isArray(src)) {
        for (const item of src) {
            if ((0, Ref_1.canonref)(item.ref) === key)
                return item.options;
        }
        return undefined;
    }
    for (const k of Object.keys(src)) {
        if ((0, Ref_1.canonref)(k) === key)
            return src[k].options;
    }
    return undefined;
}
...
function mergeone(base, over, shape) {
    if (null == over)
        return base;
...
function checkshape(shape) {
    ...
    for (const k of Object.keys(shape)) {
        const v = shape[k];
        if (!isMap(v) || undefined === v.$MERGE)
            continue;
        const d = v.$MERGE;
        if ('string' === typeof d) {
            if (-1 === MERGE_WORDS.indexOf(d)) {
                (0, Types_1.fail)('plugin_shape_invalid', 'invalid $MERGE directive at ' + k + ': ' + d, { key: k, directive: d });
            }
            continue;
        }
        if (isMap(d) && undefined !== d.deep) {
            ...
            if ('number' !== typeof n || !Number.isInteger(n) || n < 1) {
                (0, Types_1.fail)('plugin_shape_invalid', 'invalid $MERGE deep at ' + k + ': ' + JSON.stringify(n), { key: k, directive: d });
            }
            continue;
        }
        (0, Types_1.fail)('plugin_shape_invalid', 'invalid $MERGE directive at ' + k + ': ' + JSON.stringify(d), { key: k, directive: d });
    }
}
function isMap(v) {
