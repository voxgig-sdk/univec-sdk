// convert entity test - basic flow (generated from the API model).
...
public class ConvertEntityTest
{
    ...
    public void Basic()
    {
        var setup = ConvertBasicSetup(null);
        ...
        // and 4xx; set UNIVEC_TEST_CONVERT_ENTID JSON to run live.
        ...
        // CREATE
        var convertRef01Ent = client.Convert();
        var convertRef01Data = Helpers.ToMapAny(StructUtils.GetProp(
            StructUtils.GetPath(setup.Data, StructUtils.Jt("new", "convert")),
            "convert_ref01"));

        var convertRef01DataResult = convertRef01Ent.Create(convertRef01Data, null);
        convertRef01Data = Helpers.ToMapAny(convertRef01DataResult is IEntity ce ? ce.Data() : convertRef01DataResult);
        Assert.True(convertRef01Data != null, "expected create result to be a map");

    }

    private static EntityTestSetup ConvertBasicSetup(
        Dictionary<string, object?>? extra)
    {
        TestRunner.LoadEnvLocal();

        var entityDataFile = Path.Combine(TestRunner.TestDir(),
            "..", "..", ".sdk", "test", "entity", "convert",
            "ConvertTestData.json");
        ...
        var entityData = StructRunner.ConvertElement(entityDataEl)
            ...
            ?? throw new InvalidOperationException(
                "failed to parse convert test data");
        ...
        var idmap = StructUtils.Transform(
            new List<object?> { "convert01", "convert02", "convert03" },
