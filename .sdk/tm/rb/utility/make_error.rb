...
module UnivecUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = UnivecContext.new({}, nil)
    end
    op = ctx.op || UnivecOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || UnivecResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(UnivecError) ? err.msg : err.to_s
    ...
    sdk_err = UnivecError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(UnivecError)
