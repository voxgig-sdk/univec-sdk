...
{
    private static function utility(): UnivecUtility
    {
        return new UnivecUtility();
    }
    ...
    private static function ctx(array $over = []): UnivecContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new UnivecContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new UnivecOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof UnivecError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new UnivecOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = UnivecMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new UnivecOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = UnivecMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new UnivecOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = UnivecMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new UnivecSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = UnivecMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new UnivecResponse([]),
            'result' => new UnivecResult([]),
        ]);
        [, $err] = UnivecMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new UnivecSpec(['step' => 's']),
            'response' => new UnivecResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new UnivecResult([]),
        ]);
        [, $err] = UnivecMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new UnivecSpec(['step' => 's']),
            'response' => new UnivecResponse(self::resp(200, ['v' => 1])),
            'result' => new UnivecResult([]),
        ]);
        [, $err] = UnivecMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new UnivecSpec(['step' => 's']),
            'response' => new UnivecResponse(self::resp(200, ['v' => 2])),
            'result' => new UnivecResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        UnivecMakeResponse::call($ctx);
    ...
    {
        $preset = new UnivecResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new UnivecSpec([]),
            'response' => new UnivecResponse([]),
            'result' => new UnivecResult([]),
        ...
        [$got, $err] = UnivecMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new UnivecResult([])]);
        [, $err] = UnivecMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new UnivecSpec([]), 'result' => null]);
        [, $err] = UnivecMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new UnivecSpec(['step' => 's']),
            'result' => new UnivecResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new UnivecOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = UnivecMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new UnivecSpec(['step' => 's']),
            'result' => new UnivecResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new UnivecOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = UnivecMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new UnivecResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new UnivecSpec([]),
            'result' => new UnivecResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = UnivecMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = UnivecMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new UnivecError('boom', 'boom');
        $utility->fetcher = function (UnivecContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new UnivecSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = UnivecMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (UnivecContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = UnivecMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(UnivecResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (UnivecContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new UnivecSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = UnivecMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new UnivecResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new UnivecSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = UnivecMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = UnivecMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = UnivecMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new UnivecResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, UnivecDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new UnivecResult(['ok' => false])]);
        $this->expectException(UnivecError::class);
        UnivecDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new UnivecResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', UnivecMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new UnivecBaseFeature();
        $b = new UnivecBaseFeature();
        UnivecFeatureAdd::call($ctx, $a);
        UnivecFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): UnivecBaseFeature
    {
        $f = new UnivecBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        UnivecFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): UnivecContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new UnivecSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = UnivecPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new UnivecResponse(['headers' => null]),
            'result' => new UnivecResult([]),
        ]);
        UnivecResultHeaders::call($ctx);
