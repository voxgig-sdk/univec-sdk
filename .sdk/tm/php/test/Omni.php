...
//    only - a typed UnivecContext there would make every ctx
...
final class UnivecOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new UnivecContext($clean, null);
    ...
    public static function err_from_map($m): ?UnivecError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new UnivecError(is_string($code) ? $code : '', $msg);
    }
