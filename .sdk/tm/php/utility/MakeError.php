...
// Univec SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class UnivecMakeError
{
    public static function call(?UnivecContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new UnivecContext([], null);
        }
        $op = $ctx->op ?? new UnivecOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new UnivecResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof UnivecError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new UnivecError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof UnivecError) {
