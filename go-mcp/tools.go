...
type Args struct {
	Entity string         `json:"entity" jsonschema:"convert | embed | ephemeral_key | model"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.UnivecSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "univec_list",
		Description: "List records from Univec. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "univec_load",
		Description: "Load a single record from Univec. " +
...
func runOp(client *sdk.UnivecSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.UnivecSDK, name string) (sdk.UnivecEntity, error) {
	switch strings.ToLower(name) {
	case "convert":
		return client.Convert(nil), nil
	case "embed":
		return client.Embed(nil), nil
	case "ephemeral_key":
