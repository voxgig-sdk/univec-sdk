...
class UnivecSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = UnivecHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = UnivecHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!UnivecFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, UnivecFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return UnivecUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = UnivecHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = UnivecHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = UnivecHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = UnivecHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new UnivecError("graphql_error",
                "UnivecSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // PHP method names are case-insensitive, so lowercase $client->embed()
    // resolves here too.
    public function Embed($data = null)
    {
        require_once __DIR__ . '/entity/embed_entity.php';
        if ($data === null) {
            if ($this->_embed === null) {
                $this->_embed = new EmbedEntity($this, null);
            }
            return $this->_embed;
        }
        return new EmbedEntity($this, $data);
    }
