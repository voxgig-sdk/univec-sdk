...
// Univec SDK EphemeralKey entity
...
class EphemeralKeyEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new EphemeralKeyEntity($this->_client, $opts);
    }

    /**
     * @param EphemeralKey|array $args EphemeralKey data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = UnivecHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return EphemeralKey|array The current EphemeralKey data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of EphemeralKey fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = UnivecHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of EphemeralKey fields).
     */
    ...
    /**
     * Create a new EphemeralKey.
     *
     * @param EphemeralKeyCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed EphemeralKeyCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return EphemeralKey|array The created EphemeralKey as an assoc-array at the
     *   SDK boundary; throws UnivecError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = UnivecHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
