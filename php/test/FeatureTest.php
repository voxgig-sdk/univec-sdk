...
class FtCtrl extends UnivecControl
{
...
class FtClient
{
    ...
    private UnivecUtility $utility;

    public function __construct(UnivecUtility $utility, array $options)
    {
    ...
    public function get_utility(): UnivecUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UnivecContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UnivecContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UnivecConfig::make_feature($name);
    ...
    private static function buildUrl(UnivecSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UnivecError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UnivecResponse($fetched);
        }
    ...
    private function fail(UnivecContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UnivecContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
