...
class ModelDirectTest < Minitest::Test
  def test_direct_list_model
    setup = model_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-model", setup[:live] ? "live" : "unit")
    if _should_skip
...
def model_direct_setup(mockres)
  ...
  if live
    ...
    client = UnivecSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
