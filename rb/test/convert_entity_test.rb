...
class ConvertEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set UNIVEC_TEST_CONVERT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def convert_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "convert", "ConvertTestData.json")
  ...
  entid_env_raw = ENV["UNIVEC_TEST_CONVERT_ENTID"]
