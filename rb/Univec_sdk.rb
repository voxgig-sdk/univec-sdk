...
class UnivecSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = UnivecHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = UnivecHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, UnivecFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = UnivecHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = UnivecHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = UnivecHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    allow_op = VoxgigStruct.getpath(@options, "allow.op")
    {
      ...
      "err" => UnivecError.new(
        "#{op}_allow",
        "UnivecSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = UnivecHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = UnivecError.new(
        "graphql_error", "UnivecSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Embed.list / client.Embed.load({ "id" => ... })
  def Embed(data = nil)
    require_relative 'entity/embed_entity'
    EmbedEntity.new(self, data)
  end


  # Canonical facade: client.EphemeralKey.list / client.EphemeralKey.load({ "id" => ... })
  def EphemeralKey(data = nil)
    require_relative 'entity/ephemeral_key_entity'
    EphemeralKeyEntity.new(self, data)
  end
