...
class EphemeralKeyEntity
  ...
  def make
    opts = @_entopts.dup
    EphemeralKeyEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = UnivecHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [EphemeralKey, Hash] the current EphemeralKey data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = UnivecHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of EphemeralKey fields)
  def match_get
  ...
  #
  # @param reqdata [EphemeralKeyCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [EphemeralKey, Hash] the created EphemeralKey; raises UnivecError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = UnivecHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
