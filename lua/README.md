...
The Lua SDK for the Univec API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Convert()` — each with the same small set of operations (`list`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/univec-sdk/releases)),
...
```lua
-- Create
local created, err = client:Convert():create({ embeddings = {}, source_model = "example_source_model", target_model = "example_target_model" })
...
| --- | --- | --- |
| `embeddings` | `table` | Translated vectors, in the target model's dimension. |
...
Create an instance: `local embed = client:Embed(nil)`
...
| Field | Type | Description |
| --- | --- | --- |
| `embeddings` | `table` | One vector per input text, in input order. |
| `model` | `string` | Model that produced the vectors. |
| `texts` | `table` | Texts to embed. |
...
| Field | Type | Description |
| --- | --- | --- |
| `eval` | `table` | Retrieval-fidelity metrics for a convert model. |
| `executionProvider` | `string` | Hardware backend, e.g. |
| `modelCard` | `table` | Convert models only: training provenance and architecture detail. |
...
```
lua/
├── univec_sdk.lua    -- Main SDK module
...
The main module (`univec_sdk`) exports the SDK constructor
