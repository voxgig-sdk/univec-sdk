...
UnivecSDK._make_feature = _make_feature


function UnivecSDK.new(options)
  local self = setmetatable({}, UnivecSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function UnivecSDK:options_map()
...
function UnivecSDK:get_utility()
  return Utility.copy(self._utility)
end


function UnivecSDK:get_root_ctx()
  return self._rootctx
end


function UnivecSDK:prepare(fetchargs)
...
function UnivecSDK:direct(fetchargs)
...
-- by passing it.
function UnivecSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function UnivecSDK:graphql(query, variables, ctrl)
...
function UnivecSDK:Convert(data)
...
-- Idiomatic facade: client:Embed():list() / client:Embed():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function UnivecSDK:Embed(data)
  local EntityMod = require("entity.embed_entity")
  if data == nil then
    if self._embed == nil then
      self._embed = EntityMod.new(self, nil)
    end
    return self._embed
  end
  return EntityMod.new(self, data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function UnivecSDK:EphemeralKey(data)
...
function UnivecSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = UnivecSDK.new(sdkopts)
  sdk.mode = "test"
