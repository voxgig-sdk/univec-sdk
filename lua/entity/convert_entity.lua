...
local ConvertEntity = {}
ConvertEntity.__index = ConvertEntity


function ConvertEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ConvertEntity)
  self._name = "convert"
  self._client = client
...
function ConvertEntity:get_name()
  return self._name
end


function ConvertEntity:make()
  local opts = {}
...
function ConvertEntity:mark_deleted()
  self._deleted = true
end


function ConvertEntity:deleted()
  return true == self._deleted
end


function ConvertEntity:data_set(args)
  if args ~= nil then
...
function ConvertEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ConvertEntity:match_set(args)
  if args ~= nil then
...
function ConvertEntity:match_get()
...
function ConvertEntity:stream(action, args, callopts)
...
---@param reqdata ConvertCreateData
---@param ctrl? table
---@return Convert
---@return string? err
function ConvertEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ConvertEntity:_run_op(ctx, post_done)
  local utility = self._utility
