...
local EphemeralKeyEntity = {}
EphemeralKeyEntity.__index = EphemeralKeyEntity


function EphemeralKeyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EphemeralKeyEntity)
  self._name = "ephemeral_key"
  self._client = client
...
function EphemeralKeyEntity:get_name()
  return self._name
end


function EphemeralKeyEntity:make()
  local opts = {}
...
function EphemeralKeyEntity:mark_deleted()
  self._deleted = true
end


function EphemeralKeyEntity:deleted()
  return true == self._deleted
end


function EphemeralKeyEntity:data_set(args)
  if args ~= nil then
...
function EphemeralKeyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EphemeralKeyEntity:match_set(args)
  if args ~= nil then
...
function EphemeralKeyEntity:match_get()
...
function EphemeralKeyEntity:stream(action, args, callopts)
...
---@param reqdata EphemeralKeyCreateData
---@param ctrl? table
---@return EphemeralKey
---@return string? err
function EphemeralKeyEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function EphemeralKeyEntity:_run_op(ctx, post_done)
  local utility = self._utility
