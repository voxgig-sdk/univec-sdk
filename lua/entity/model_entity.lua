...
local ModelEntity = {}
ModelEntity.__index = ModelEntity


function ModelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ModelEntity)
  self._name = "model"
  self._client = client
...
function ModelEntity:get_name()
  return self._name
end


function ModelEntity:make()
  local opts = {}
...
function ModelEntity:mark_deleted()
  self._deleted = true
end


function ModelEntity:deleted()
  return true == self._deleted
end


function ModelEntity:data_set(args)
  if args ~= nil then
...
function ModelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ModelEntity:match_set(args)
  if args ~= nil then
...
function ModelEntity:match_get()
...
function ModelEntity:stream(action, args, callopts)
...
function ModelEntity:_run_op(ctx, post_done)
  local utility = self._utility
