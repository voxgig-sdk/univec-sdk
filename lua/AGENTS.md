# Univec Lua — Agent Guide

The Lua client for the Univec API. This directory is **generated** — do not edit it by hand; change the model/template/component in `.sdk/` and regenerate. See the [project guide](../AGENTS.md) for the full workflow and the aontu model language.
...
| **cache** — Response caching for safe read requests | `feature/cache_feature.lua` | — |
| **clienttrack** — Client identity and per-request correlation headers | `feature/clienttrack_feature.lua` | `PostConstruct`, `PreRequest` |
| **cost** — Cost tracking and spend budget for API calls | `feature/cost_feature.lua` | `PreDone`, `PrePoint`, `PreUnexpected` |
...
| **netsim** — Network behaviour simulation for offline testing (latency, failures, outages) | `feature/netsim_feature.lua` | — |
| **paging** — Pagination signals for list operations | `feature/paging_feature.lua` | `PreRequest`, `PreResult` |
| **proxy** — Outbound HTTP(S) proxy routing | `feature/proxy_feature.lua` | — |
| **ratelimit** — Client-side rate limiting via a token bucket | `feature/ratelimit_feature.lua` | — |
| **rbac** — Client-side role/permission enforcement | `feature/rbac_feature.lua` | `PrePoint` |
| **retry** — Automatic retry of transient failures with exponential backoff | `feature/retry_feature.lua` | — |
| **secrets** — Secret access: resolve the API credential through a provider chain, and exchange a refresh token for short-lived access tokens | `feature/secrets_feature.lua` | `PreSpec` |
| **streaming** — Incremental streaming of list results via async iteration | `feature/streaming_feature.lua` | `PreResult` |
