...
use univec_sdk::utility::voxgigstruct::*;
...
pub fn test_json() -> Value {
    ...
    univec_sdk::utility::jsonparse::json_parse(&txt).expect("parse test.json")
}
...
pub fn matchval(check: &Value, base: &Value) -> bool {
    ...
    if let Value::Str(cs) = check {
        ...
        if let Some(rem) = cs.strip_prefix('/').and_then(|s| s.strip_suffix('/')) {
            if let Ok(re) = univec_sdk::utility::voxgigstruct::re::Regex::new(rem) {
        ...
        for (i, entry) in entries.iter().enumerate() {
            ...
            match result {
                Err(msg) => {
                    ...
                    {
                        univec_sdk::utility::voxgigstruct::re::Regex::new(rem)
