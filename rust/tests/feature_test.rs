...
use univec_sdk::core::helpers::{get_bool, get_str, getp, ja, jo, vfn};
use univec_sdk::feature::audit::AuditFeature;
use univec_sdk::feature::cache::CacheFeature;
use univec_sdk::feature::clienttrack::ClienttrackFeature;
use univec_sdk::feature::debug::DebugFeature;
use univec_sdk::feature::idempotency::IdempotencyFeature;
use univec_sdk::feature::metrics::MetricsFeature;
use univec_sdk::feature::netsim::NetsimFeature;
use univec_sdk::feature::paging::PagingFeature;
use univec_sdk::feature::proxy::ProxyFeature;
use univec_sdk::feature::ratelimit::RatelimitFeature;
use univec_sdk::feature::rbac::RbacFeature;
use univec_sdk::feature::retry::RetryFeature;
use univec_sdk::feature::streaming::StreamingFeature;
use univec_sdk::feature::telemetry::TelemetryFeature;
use univec_sdk::feature::timeout::TimeoutFeature;
use univec_sdk::utility::voxgigstruct as vs;
use univec_sdk::{Context, FeatureRef, Value};

fn fr<T: univec_sdk::Feature + 'static>(f: &Rc<RefCell<T>>) -> FeatureRef {
...
fn feature_retry_retries_transport_error_then_returns_it() {
    ...
    let nc = n.clone();
    let server: univec_sdk::FetcherFn = Rc::new(move |ctx: &Rc<Context>, _u, _f| {
        *nc.borrow_mut() += 1;
...
fn feature_retry_retries_nil_transport_result() {
    ...
    let n = Rc::new(RefCell::new(0));
    let nc = n.clone();
    let server: univec_sdk::FetcherFn = Rc::new(move |_c: &Rc<Context>, _u, _f| {
        *nc.borrow_mut() += 1;
        if *nc.borrow() < 2 {
            Ok(Value::Noval)
        } else {
...
fn feature_timeout_slow_request_times_out() {
    ...
    let server: univec_sdk::FetcherFn = Rc::new(|_c: &Rc<Context>, _u, _f| {
...
fn feature_station_inert_without_open_station() {
    ...
    let f = univec_sdk::make_feature("station");
    assert_eq!(
        "station",
        f.borrow().name(),
