...
fn util_with(client: &Rc<UnivecSDK>, fetcher: FetcherFn) -> Rc<Utility> {
...
fn pipeline_feature_add_ordering_before_after_replace() {
    ...
    let names = |client: &Rc<UnivecSDK>| -> String {
        client
            .features
            .borrow()
            .iter()
...
use univec_sdk::core::helpers::get_str;
