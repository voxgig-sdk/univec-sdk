...
#[test]
fn embed_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.embed(Value::Noval);
    assert_eq!(ent.get_name(), "embed");
}

#[test]
fn embed_entity_basic() {
    let setup = embed_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("embed.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set UNIVEC_TEST_EMBED_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let embed_ref01_ent = client.embed(Value::Noval);
    let embed_ref01_data = to_map(&getp(
        &getpath(&["new", "embed"], &setup.data),
        "embed_ref01",
    ));

    let embed_ref01_data_result = embed_ref01_ent
        .create(embed_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let embed_ref01_data = to_map(&embed_ref01_data_result.data(None));
    assert!(
        matches!(embed_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn embed_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("embed");
    entity_data_file.push("EmbedTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("embed01"), Value::str("embed02"), Value::str("embed03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("UNIVEC_TEST_EMBED_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("UNIVEC_TEST_EMBED_ENTID", idmap.clone()),
        ("UNIVEC_TEST_LIVE", Value::str("FALSE")),
        ("UNIVEC_TEST_EXPLAIN", Value::str("FALSE")),
        ("UNIVEC_APIKEY", Value::str("")),
    ]));

    let idmap_resolved = match to_map(&getp(&env, "UNIVEC_TEST_EMBED_ENTID")) {
