// UnivecError: the SDK error type (mirrors go core/error.go). The
// pipeline error discipline is Result<T, UnivecError> throughout.

use crate::utility::voxgigstruct::Value;

#[derive(Clone, Debug)]
pub struct UnivecError {
    pub sdk: String,
    pub code: String,
    pub msg: String,
...
impl UnivecError {
    pub fn new(code: &str, msg: &str) -> UnivecError {
        UnivecError {
            sdk: "Univec".to_string(),
            code: code.to_string(),
            msg: msg.to_string(),
            result: Value::Noval,
            spec: Value::Noval,
...
impl std::fmt::Display for UnivecError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(&self.msg)
    }
}

impl std::error::Error for UnivecError {}
