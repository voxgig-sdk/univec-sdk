// Model entity client (generated — mirrors the go Entity fragment).
...
impl ModelEntity {
    pub fn new(client: &Rc<UnivecSDK>, entopts: Value) -> Rc<ModelEntity> {
...
impl Entity for ModelEntity {
    ...
    fn make(&self) -> Rc<dyn Entity> {
        ...
        ModelEntity::new(&self.client, opts) as Rc<dyn Entity>
    }
...
impl UnivecEntity for ModelEntity {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, UnivecError> {
        let ctx = self.utility.make_context(
            CtxSpec {
        ...
        if let Value::List(list) = &out {
            for entry in list.borrow().iter() {
                let ent = ModelEntity::new(&self.client, vs::clone(&self.entopts));
    ...
    fn create(self: &Rc<Self>, _reqdata: Value, _ctrl: Value) -> Result<Rc<Self>, UnivecError> {
