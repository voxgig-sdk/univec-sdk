...
import (
	...
	omni "github.com/voxgig-sdk/univec-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UnivecError); is {
				out := map[string]any{
					"name":    "UnivecError",
...
func omniCtx(arg any, client *sdk.UnivecSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UnivecSDK); is {
			client = live
