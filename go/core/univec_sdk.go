...
func (sdk *UnivecSDK) OptionsMap() map[string]any {
...
func (sdk *UnivecSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *UnivecSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *UnivecSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *UnivecSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *UnivecSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *UnivecSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("UnivecSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *UnivecSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *UnivecSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("UnivecSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func (sdk *UnivecSDK) Convert(data map[string]any) UnivecEntity {
	return NewConvertEntityFunc(sdk, data)
}


// Embed returns a Embed entity bound to this client.
// Idiomatic usage: client.Embed(nil).List(nil, nil) or
// client.Embed(nil).Load(map[string]any{"id": ...}, nil).
func (sdk *UnivecSDK) Embed(data map[string]any) UnivecEntity {
	return NewEmbedEntityFunc(sdk, data)
}
...
func (sdk *UnivecSDK) EphemeralKey(data map[string]any) UnivecEntity {
...
func (sdk *UnivecSDK) Model(data map[string]any) UnivecEntity {
