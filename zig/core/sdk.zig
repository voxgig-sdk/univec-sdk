// UnivecSDK client (generated — mirrors the go/rust Main fragment).
...
pub const UnivecSDK = struct {
    ...
    pub fn options_map(self: *UnivecSDK) Value {
        return h.clone(self.options);
    }

    pub fn get_utility(self: *UnivecSDK) *Utility {
        return Utility.copy(self.sdkUtility);
    }

    pub fn get_root_ctx(self: *UnivecSDK) *Context {
        return self.rootctx orelse unreachable;
    }

    pub fn prepare(self: *UnivecSDK, fetchargs_in: Value) E!Value {
    ...
    pub fn direct(self: *UnivecSDK, fetchargs_in: Value) Value {
    ...
    fn op_allowed(self: *UnivecSDK, op: []const u8) bool {
    ...
    fn op_denied(self: *UnivecSDK, op: []const u8) Value {
        const allow: []const u8 = switch (h.getpath(&.{ "allow", "op" }, self.options)) {
            .string => |s| s,
            else => "",
        };
        const msg = std.fmt.allocPrint(h.A(),
            "UnivecSDK: {s}: operation not allowed by" ++
    ...
    // gate by passing it.
    fn raw_request(self: *UnivecSDK, fetchargs_in: Value) Value {
    ...
    pub fn graphql(
        self: *UnivecSDK, query: []const u8, variables: Value, ctrl: Value,
    ) Value {
    ...
    pub fn convert(self: *@This(), entopts: Value) *@import("../entity/convert.zig").ConvertEntity {
        return @import("../entity/convert.zig").ConvertEntity.new(self, entopts);
    }

    /// Embed entity bound to this client.
    pub fn embed(self: *@This(), entopts: Value) *@import("../entity/embed.zig").EmbedEntity {
        return @import("../entity/embed.zig").EmbedEntity.new(self, entopts);
    }

    /// EphemeralKey entity bound to this client.
    pub fn ephemeral_key(self: *@This(), entopts: Value) *@import("../entity/ephemeral_key.zig").EphemeralKeyEntity {
        return @import("../entity/ephemeral_key.zig").EphemeralKeyEntity.new(self, entopts);
    }

    /// Model entity bound to this client.
    pub fn model(self: *@This(), entopts: Value) *@import("../entity/model.zig").ModelEntity {
        return @import("../entity/model.zig").ModelEntity.new(self, entopts);
    }

};

pub fn test_sdk(testopts_in: Value, sdkopts_in: Value) *UnivecSDK {
    ...
    const sdk = UnivecSDK.new(sdkopts);
    sdk.mode = "test";
