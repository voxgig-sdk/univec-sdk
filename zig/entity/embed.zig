// Embed entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EmbedEntity = struct {
    ...
    pub fn new(client: *sdk.UnivecSDK, entopts_in: Value) *EmbedEntity {
        ...
        const e = h.A().create(EmbedEntity) catch unreachable;
        e.* = .{
            .name = "embed",
            .client = client,
    ...
    fn ent_ctx(self: *EmbedEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *EmbedEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *EmbedEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *EmbedEntity, ctx: *Context, post_done: *const fn (*EmbedEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *EmbedEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *EmbedEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *EmbedEntity, ctx: *Context, post_done: *const fn (*EmbedEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *EmbedEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *EmbedEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return EmbedEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *EmbedEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *EmbedEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *EmbedEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *EmbedEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *EmbedEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *EmbedEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *EmbedEntity, ctx: *Context) void {
    ...
    pub fn update(self: *EmbedEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *EmbedEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
