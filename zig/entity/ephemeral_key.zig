// EphemeralKey entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EphemeralKeyEntity = struct {
    name: []const u8 = "ephemeral_key",
    ...
    pub fn new(client: *sdk.UnivecSDK, entopts_in: Value) *EphemeralKeyEntity {
        ...
        const e = h.A().create(EphemeralKeyEntity) catch unreachable;
        e.* = .{
            .name = "ephemeral_key",
            .client = client,
    ...
    fn ent_ctx(self: *EphemeralKeyEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *EphemeralKeyEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *EphemeralKeyEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *EphemeralKeyEntity, ctx: *Context, post_done: *const fn (*EphemeralKeyEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *EphemeralKeyEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *EphemeralKeyEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *EphemeralKeyEntity, ctx: *Context, post_done: *const fn (*EphemeralKeyEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *EphemeralKeyEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *EphemeralKeyEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return EphemeralKeyEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *EphemeralKeyEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *EphemeralKeyEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *EphemeralKeyEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *EphemeralKeyEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *EphemeralKeyEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *EphemeralKeyEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *EphemeralKeyEntity, ctx: *Context) void {
    ...
    pub fn update(self: *EphemeralKeyEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *EphemeralKeyEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
