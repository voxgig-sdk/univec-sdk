// Model entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ModelEntity = struct {
    ...
    pub fn new(client: *sdk.UnivecSDK, entopts_in: Value) *ModelEntity {
        ...
        const e = h.A().create(ModelEntity) catch unreachable;
        e.* = .{
            .name = "model",
            .client = client,
    ...
    fn ent_ctx(self: *ModelEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ModelEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ModelEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *ModelEntity, ctx: *Context, post_done: *const fn (*ModelEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *ModelEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *ModelEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *ModelEntity, ctx: *Context, post_done: *const fn (*ModelEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ModelEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ModelEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ModelEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ModelEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ModelEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ModelEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *ModelEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn list(self: *ModelEntity, reqmatch: Value, ctrl: Value) EntListResult {
        ...
        var items: std.ArrayList(*ModelEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = ModelEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*ModelEntity{} };
    }
    
    fn list_post_done(self: *ModelEntity, ctx: *Context) void {
    ...
    pub fn create(self: *ModelEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *ModelEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *ModelEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
