...
defmodule Univec do
  ...
  alias Univec.{Utility, Context, Spec}
  ...
  def new(options \\ nil) do
    ...
    config = Univec.Config.shared_config()
    ...
    if S.islist(feature_order) and S.size(feature_order) > 0 do
      Enum.each(0..(S.size(feature_order) - 1), fn i ->
        ...
        if fom != nil and S.getprop(fom, "active") == true do
          Utility.feature_add(rootctx, Univec.Features.make_feature(fname))
        end
      end)
    end
      ...
      _ ->
        ...
        cond do
          ...
          S.ismap(fetched) ->
            ...
            content_length =
              if S.ismap(headers), do: Univec.Feature.header_get(headers, "content-length"), else: nil
  ...
  def graphql(client, query, variables \\ nil, ctrl \\ nil) do
    if not op_allowed?(client, "graphql") do
      ...
      if count == 0 do
        ...
        S.setprop(res, "err", "UnivecSDK: graphql: " <> msg)
  ...
  @doc "Entity factory for convert."
  def convert(client, entopts \\ nil) do
    Univec.Entity.Convert.new(client, entopts)
  end

  @doc "Entity factory for embed."
  def embed(client, entopts \\ nil) do
    Univec.Entity.Embed.new(client, entopts)
  end

  @doc "Entity factory for ephemeral_key."
  def ephemeral_key(client, entopts \\ nil) do
    Univec.Entity.EphemeralKey.new(client, entopts)
  end

  @doc "Entity factory for model."
  def model(client, entopts \\ nil) do
    Univec.Entity.Model.new(client, entopts)
  end
