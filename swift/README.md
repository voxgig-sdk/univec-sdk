...
The Swift SDK for the Univec API — an entity-oriented client following idiomatic Swift conventions.
...
(`swift/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/univec-sdk/releases)) by adding it to
your `Package.swift`:
...
let model = try client.Model().list(nil, nil)
...
```swift
let client = UnivecSDK.testSDK(testopts, sdkopts)
```
...
Create an instance: `let convert = client.Convert()`
...
Create an instance: `let embed = client.Embed()`
...
```swift
let embed = try client.Embed().create(VMap([
    ("embeddings", .list([])),  // [Value]
...
Create an instance: `let ephemeralKey = client.EphemeralKey()`
...
```swift
let ephemeralKey = try client.EphemeralKey().create(VMap([
...
Create an instance: `let model = client.Model()`
...
| Field | Type | Description |
| --- | --- | --- |
| `eval` | `VMap` | Retrieval-fidelity metrics for a convert model. |
| `executionProvider` | `String` | Hardware backend, e.g. |
| `modelCard` | `VMap` | Convert models only: training provenance and architecture detail. |
...
A `UnivecTypes.swift` file of reference `struct` types is also
...
└── Tests/UnivecSdkTests/    -- Test suites (XCTest)
```

The main client class (`UnivecSDK`, under `Sources/UnivecSdk/core`)
