...
**Features** (19): `audit`, `cache`, `clienttrack`, `cost`, `debug`, `idempotency`, `log`, `metrics`, `netsim`, `paging`, `proxy`, `ratelimit`, `rbac`, `retry`, `secrets`, `streaming`, `telemetry`, `test`, `timeout`.
...
**Entities** (4): `Convert`, `Embed`, `EphemeralKey`, `Model`.
