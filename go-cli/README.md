...
boru-driven command-line client **and** interactive REPL for the Univec
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/univec-cli)
make build
...
UNIVEC_BASE=https://api.example.com ./univec-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/univec-cli
   ```
...
Run with no arguments to open a REPL (prompt `univec>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `convert`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/univec-cli`. |
...
(`./univec-cli <expr>`) and interactively (the REPL), and expressions compose the
