// Command Univec-cli is a CLI + REPL for the Univec SDK,
...
const entitiesHelp = "convert embed ephemeral_key model"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      UNIVEC_APIKEY (api key), UNIVEC_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: UNIVEC_APIKEY carries the API key and
	// UNIVEC_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("UNIVEC_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("UNIVEC_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewUnivecSDK(opts)
