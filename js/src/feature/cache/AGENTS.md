...
runtime for the `cache` feature in the JavaScript target — do
...
| Runtime template | `.sdk/tm/js/src/feature/cache/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/js/src/feature/cache/`, then regenerate.
