# AuditFeature — Agent Guide

Structured audit trail of operations (v0.0.1).
...
runtime for the `audit` feature in the JavaScript target — do
...
| Part | Path |
| --- | --- |
| Model definition | `.sdk/model/feature/audit.aon` (name, title, version, `config.options.active`, the `hook` map, per-language `deps`) |
| Registered in | `.sdk/model/feature/feature-index.aon` (`@"audit.aon"`) |
| Runtime template | `.sdk/tm/js/src/feature/audit/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Turn hooks on/off**: edit the `hook` map in
  `.sdk/model/feature/audit.aon` (`<Stage>: active: true|false`).
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/js/src/feature/audit/`, then regenerate.
