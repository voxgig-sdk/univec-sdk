# RbacFeature — Agent Guide

Client-side role/permission enforcement (v0.0.1).
...
runtime for the `rbac` feature in the JavaScript target — do
...
| Part | Path |
| --- | --- |
| Model definition | `.sdk/model/feature/rbac.aon` (name, title, version, `config.options.active`, the `hook` map, per-language `deps`) |
| Registered in | `.sdk/model/feature/feature-index.aon` (`@"rbac.aon"`) |
| Runtime template | `.sdk/tm/js/src/feature/rbac/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Turn hooks on/off**: edit the `hook` map in
  `.sdk/model/feature/rbac.aon` (`<Stage>: active: true|false`).
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/js/src/feature/rbac/`, then regenerate.
