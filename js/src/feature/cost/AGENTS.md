...
runtime for the `cost` feature in the JavaScript target — do
...
| Runtime template | `.sdk/tm/js/src/feature/cost/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/js/src/feature/cost/`, then regenerate.
