...
runtime for the `secrets` feature in the JavaScript target — do
...
| Runtime template | `.sdk/tm/js/src/feature/secrets/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/js/src/feature/secrets/`, then regenerate.
