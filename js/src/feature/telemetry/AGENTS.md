# TelemetryFeature — Agent Guide

Distributed tracing spans with W3C trace-context propagation (v0.0.1).
...
runtime for the `telemetry` feature in the JavaScript target — do
...
| Part | Path |
| --- | --- |
| Model definition | `.sdk/model/feature/telemetry.aon` (name, title, version, `config.options.active`, the `hook` map, per-language `deps`) |
| Registered in | `.sdk/model/feature/feature-index.aon` (`@"telemetry.aon"`) |
| Runtime template | `.sdk/tm/js/src/feature/telemetry/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Turn hooks on/off**: edit the `hook` map in
  `.sdk/model/feature/telemetry.aon` (`<Stage>: active: true|false`).
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/js/src/feature/telemetry/`, then regenerate.
