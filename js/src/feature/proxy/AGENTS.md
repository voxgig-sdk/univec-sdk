# ProxyFeature — Agent Guide

Outbound HTTP(S) proxy routing (v0.0.1).
...
runtime for the `proxy` feature in the JavaScript target — do
...
| Part | Path |
| --- | --- |
| Model definition | `.sdk/model/feature/proxy.aon` (name, title, version, `config.options.active`, the `hook` map, per-language `deps`) |
| Registered in | `.sdk/model/feature/feature-index.aon` (`@"proxy.aon"`) |
| Runtime template | `.sdk/tm/js/src/feature/proxy/` (copied here on `generate`; `FEATURE_Name`/`FEATURE_VERSION` substituted) |
...
- **Turn hooks on/off**: edit the `hook` map in
  `.sdk/model/feature/proxy.aon` (`<Stage>: active: true|false`).
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/js/src/feature/proxy/`, then regenerate.
