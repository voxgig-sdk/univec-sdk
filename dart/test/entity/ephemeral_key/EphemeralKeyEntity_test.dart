...
void tests() {
  describe('EphemeralKeyEntity', () {
    test('instance', (t) async {
      final testsdk = UnivecSDK.test();
      final ent = testsdk.EphemeralKey();
      ok(null != ent);
    });
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'ephemeral_key.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final ephemeral_key_ref01_ent = client.EphemeralKey();
      dynamic ephemeral_key_ref01_data = setup['data']['new']['ephemeral_key']['ephemeral_key_ref01'];

      ephemeral_key_ref01_data = (await ephemeral_key_ref01_ent.create(ephemeral_key_ref01_data)).data();
      ok(null != ephemeral_key_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['ephemeral_key01', 'ephemeral_key02', 'ephemeral_key03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['UNIVEC_TEST_EPHEMERAL_KEY_ENTID'];
