  ...
  describe('EmbedEntity', () {
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'embed.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final embed_ref01_ent = client.Embed();
      dynamic embed_ref01_data = setup['data']['new']['embed']['embed_ref01'];

      embed_ref01_data = (await embed_ref01_ent.create(embed_ref01_data)).data();
      ok(null != embed_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['embed01', 'embed02', 'embed03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['UNIVEC_TEST_EMBED_ENTID'];
