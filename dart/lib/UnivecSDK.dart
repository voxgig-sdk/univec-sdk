...
import 'entity/ConvertEntity.dart';
import 'entity/EmbedEntity.dart';
import 'entity/EphemeralKeyEntity.dart';
import 'entity/ModelEntity.dart';


export 'UnivecTypes.dart';
export 'entity/ConvertEntity.dart';
export 'entity/EmbedEntity.dart';
export 'entity/EphemeralKeyEntity.dart';
export 'entity/ModelEntity.dart';
...
import 'Config.dart';
import 'Spec.dart';
// UnivecEntityBase / UnivecError / BaseFeature are re-exported below;
...
// `utility` (UnivecTypes.dart) — exporting both from this library makes
// the name ambiguous and NO dart SDK for such an API compiles:
//
//   Error: 'Utility' is exported from both 'lib/UnivecTypes.dart'
...
export 'UnivecEntityBase.dart' show UnivecEntityBase;
export 'UnivecError.dart' show UnivecError;
...
class UnivecSDK {
  ...
  dynamic _opDenied(String op) {
    ...
    return {
      'ok': false,
      'err': Exception('UnivecSDK: $op: operation not allowed by'
  ...
  Future<dynamic> graphql(String query,
    ...
    if (errors is List && 0 < errors.length) {
      ...
      res['ok'] = false;
      res['err'] = Exception('UnivecSDK: graphql: '
  ...
  ConvertEntity Convert([dynamic entopts]) {
    return ConvertEntity(this, entopts);
  }


  // Entity access: `client.Embed().list()` / `client.Embed().load({'id': ...})`.
  EmbedEntity Embed([dynamic entopts]) {
    return EmbedEntity(this, entopts);
  }


  // Entity access: `client.EphemeralKey().list()` / `client.EphemeralKey().load({'id': ...})`.
  EphemeralKeyEntity EphemeralKey([dynamic entopts]) {
    return EphemeralKeyEntity(this, entopts);
  }


  // Entity access: `client.Model().list()` / `client.Model().load({'id': ...})`.
  ModelEntity Model([dynamic entopts]) {
    return ModelEntity(this, entopts);
  }



  static UnivecSDK test([dynamic testoptsarg, dynamic sdkoptsarg]) {
    ...
    final testsdk = UnivecSDK(sdkopts);
    testsdk.mode = 'test';

    return testsdk;
  }

  UnivecSDK tester([dynamic testopts, dynamic sdkopts]) {
  ...
  @override
  String toString() {
    return 'Univec ' + _utility.struct.jsonify(toJSON());
  }
}

typedef SDK = UnivecSDK;
